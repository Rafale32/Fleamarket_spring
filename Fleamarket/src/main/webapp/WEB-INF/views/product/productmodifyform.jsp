...
<body>
  ...
  <form action="/fleamarket/product/productmodify"
    onsubmit="return addCheck()" method="post"
    enctype="multipart/form-data" name="multipleUpload">
    ...
    <div id="imgfiles">
      <input type="file" class="file_tag" id="inputimgs"
        name="inputimgs" />
    </div>
    <div>
      <div class="imgs_wrap">
        <c:forEach items="${bean.itemDTO.itemImgList }" var="tmp"
          varStatus="status">
          <img
            src="/fleamarket/resources/product/upload${tmp.thum_Img }"
            class="img${status.index }">
          <button
            value="${tmp.thum_Img }|${tmp.big_Img }|${status.index }"
            class="btn" id="btn${status.index }">그림 삭제하기</button>
        </c:forEach>
      </div>
      <div class="uploadedList"></div>
    </div>
    <div class="hiddenimg"></div>
    <input type="hidden" value="${bean.itemDTO.itemboard_No }"
      name="itemboard_No">
      ...
      <div>
        <select name="subname" id="subname">
          ...
          <c:forEach var="tmp" items="${bean.cateList }">
            <c:choose>
              <c:when
                test="${tmp.category_no == bean.itemDTO.category_No}">
    ...
    <div>
      ...
      <div>
        <input type="text" placeholder="선호 거래 지역" autocomplete="on"
          value="${bean.itemDTO.itemboard_Local }"
          name="itemboard_Local">
    ...
    <div>
      ...
      <div>
        <input type="text" class="form-control" id="input_name"
          placeholder="제목 (최대 100자)" autocomplete="on"
          name="itemboard_Title"
          value="${bean.itemDTO.itemboard_Title }" />
      </div>
    </div>
    <div>
      <label>가격:</label>
      ...
      <div>
        <input type="number" placeholder="10000 (가격은 숫자로 만 입력)"
          value="${bean.itemDTO.item_Price }" name="item_Price">

        <div>
          <label> <c:if
              test="${bean.itemDTO.item_Delivery_B == 1 }">
              <input type="checkbox" name="deliveryIs" checked="checked">
            </c:if> <c:if test="${bean.itemDTO.item_Delivery_B == 0 }">
        ...
        <div>
          <label> <c:if
              test="${bean.itemDTO.itemboard_Change_b == 1 }">
              <input id="input_exchg" type="checkbox" name="changeIs"
                checked="checked">
            </c:if> <c:if test="${bean.itemDTO.itemboard_Change_b == 0 }">
              <input id="input_exchg" type="checkbox" name="changeIs">
            </c:if> 교환가능
          </label>
        </div>
    ...
    <div>
      ...
      <div>
        <input type="text" placeholder="선택 사항 (예: 아이폰, 케이스)"
          value="${bean.itemDTO.tag_Name }" width="140px"
          name="tag_Name" />
      </div>
    </div>
    <label>수량:</label> <input type="number" placeholder="1"
      value="${bean.itemDTO.item_Amount }" name="item_Amount" /> <br>
