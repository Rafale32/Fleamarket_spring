...
<body>
	...
	<table border="1">
		...
		<c:choose>
			...
			<c:when test="${param.member_email != '' }">
				<td><a href="/fleamarket/product/productmodifyform?itemboard_No=${tmp.itemboard_No }">수정</a> 
	...
	<c:if test="${member.member_email != null && bean.itemDTO.store_name == null}">
		<c:if test="${bean.pageModel.startPage>5 }">
			<a href="/fleamarket/product/productlist?member_email=${member.member_email }&pageNum=${bean.pageModel.startPage - 1 }">[이전]</a>
		</c:if>
		...
		<c:forEach var="pageNo"  begin="${bean.pageModel.startPage}" end="${bean.pageModel.endPage }">
			...
			<a href="/fleamarket/product/productlist?member_email=${member.member_email }&pageNum=${pageNo }">[${pageNo }]</a>
			<c:if test="${bean.pageModel.requestPage == pageNo }"></b></c:if>
		</c:forEach>
		
		<c:if test="${bean.pageModel.endPage < bean.pageModel.totalPageCount}">
			<a href="/fleamarket/product/productlist?member_email=${member.member_email }&pageNum=${bean.pageModel.startPage + 5 }">[이후]</a>
		</c:if>
	</c:if>
	
	<c:if test="${param.store_name != null }">
		<c:if test="${bean.pageModel.startPage>5 }">
			<a href="/fleamarket/product/productlist?store_Name=${bean.itemDTO.store_Name }&pageNum=${bean.pageModel.startPage - 1 }">[이전]</a>
		</c:if>
		...
		<c:if test="${bean.pageModel.endPage < bean.pageModel.totalPageCount}">
			<a href="/fleamarket/product/productlist?store_Name=${bean.itemDTO.store_Name }&pageNum=${bean.pageModel.startPage + 5 }">[이후]</a>
		</c:if>
	</c:if>
