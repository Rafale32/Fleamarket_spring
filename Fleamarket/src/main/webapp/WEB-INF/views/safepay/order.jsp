...
<body>
  ...
  <form action="order" method="post" onsubmit="return payCheck()">
    ...
    <input type="hidden" name="oorder_no" value="${oorder_no}">
    <div>
    <table class="order_table" id="delivery_table">
      <tr>
      <td colspan="3"><h1>주문/결제</h1></td>
      ...
      <tr>
        ...
        <td class="right_text">
          <input type="button" class="btn btn-success" value="주소검색" onclick="search_address()">
        </td>
      </tr>
      <tr>
        <td colspan="2">
  ...
  <div>
    ...
    <tr><td colspan="3"><h2>포인트</h2></td></tr>
    <tr>
    ...
    <tr>
      ...
      <!--  버튼 클릭시 포인트 보이는 부분 변경 20180818 재헌 -->
        <input type="button" id="allUseButton" class="btn btn-info"
               value="전액사용" onclick="allPoint(${havePoint})">
      </td>
    </tr>
  </table>
  </div>
  ...
  <div>
    ...
    <tr><td colspan="2"><h2>결제금액</h2></td></tr>
    <tr>
      <td>상품금액</td>
  ...
  <input type="hidden" id="finalPoint2" name="pay_usepoint" value="${finalPoint2}">
  <!--     총 금액 -->
  <fmt:parseNumber var="total2" value="${total}" integerOnly="true" />
  <input type="hidden" id="total2" name="pay_price" value="${total2}">
  <!--     계산된 포인트 -->
  <fmt:parseNumber var="point" value="${havePoint-finalPoint+(total*0.001) }" integerOnly="true" />
  <input type="hidden" id="point" name="member_point" value="${point}">
  <hr>
  <!-- 결제 하단 -->
  <div>
